...
```sql
-- Intentar insertar con nombre de asignatura duplicado (violará restricción UNIQUE)
CALL p_insert_degree_subject(
    'Grado en Ciberseguridad',
    4,
    'Aprendizaje Automático',  -- Nombre duplicado
    'AA2',
    6,
    3,
    'Obligatoria'
...
-- ¡El grado SÍ se insertó aunque la asignatura falló! (PROBLEMA)
```

**Problema identificado:**
- El grado se inserta correctamente.
- La asignatura falla (nombre duplicado).
- El procedimiento termina con error pero el grado queda insertado.
- **Inconsistencia**: tenemos un grado sin su asignatura asociada.
...
```sql
-- Intentar insertar con nombre de asignatura duplicado
CALL p_insert_degree_subject_transactional(
    'Grado en Ciberseguridad',
    4,
    'Aprendizaje Automático',  -- Nombre duplicado (violará UNIQUE)
    'AA2',
    6,
    3,
    'Obligatoria'
...
SELECT * FROM degrees WHERE degree_name = 'Grado en Ciberseguridad';
-- ¡Ahora el grado NO se insertó! (CORRECTO)
```
...
3. **Ejercicio 3**: Crea un procedimiento transaccional `p_batch_insert_grades` que inserte múltiples notas (recibirlas como parámetros separados por comas o como tabla temporal) y garantice que o se insertan todas o ninguna.

4. **Ejercicio 4**: Experimenta con transacciones concurrentes: abre dos sesiones, inicia una transacción en una que actualice una nota, y desde la otra sesión intenta leer esa nota antes y después del COMMIT.

## Resumen

En este laboratorio has aprendido a:

- ✅ Comprender el concepto de transacción y las propiedades ACID.
- ✅ Controlar el comportamiento de AUTOCOMMIT.
- ✅ Usar `START TRANSACTION`, `COMMIT` y `ROLLBACK`.
- ✅ Implementar procedimientos transaccionales con manejo de excepciones.
- ✅ Garantizar atomicidad en operaciones relacionadas.
- ✅ Observar el aislamiento de transacciones concurrentes.
- ✅ Aplicar buenas prácticas en el manejo de transacciones.

Las transacciones son fundamentales para mantener la integridad y consistencia de los datos en bases de datos multiusuario, especialmente cuando múltiples operaciones deben tratarse como una unidad atómica.
