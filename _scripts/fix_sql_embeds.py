...
pattern_placeholder = re.compile(r"\{\%\s*include\s+sql-embed\.html\s+src='https://raw\.githubusercontent\.com/IISSI-US/silence-db/main/'\s+label=''(?P<rest>\s+(?:collapsed|collapsible)=true)?\s*\%\}")
...
def main():
    ...
    for bak in root.rglob('index.md.bak'):
        ...
        for md in dir_path.glob('*.md'):
            ...
            def repl_div(m):
                p = m.group(1)
                return f"{{% include sql-embed.html src='https://raw.githubusercontent.com/IISSI-US/silence-db/main/{p}' label='{p}' %}}"
            ...
            def repl_ph(m):
                ...
                return f"{{% include sql-embed.html src='https://raw.githubusercontent.com/IISSI-US/silence-db/main/{p}' label='{p}'{norm} %}}"
    ...
    for md in root.rglob('*.md'):
        txt = md.read_text(encoding='utf-8')
        new_txt = pattern_div.sub(lambda m: f"{{% include sql-embed.html src='https://raw.githubusercontent.com/IISSI-US/silence-db/main/{m.group(1)}' label='{m.group(1)}' %}}", txt)
        if new_txt != txt:
